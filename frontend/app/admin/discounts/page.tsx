...
export default function AdminDiscounts(){
  ...
  async function load(){ const r=await fetch(`${API}/admin/discounts`,{ credentials:'include' }); const d=await r.json(); setList(d.codes||[]); }
  useEffect(()=>{ load(); },[]);
  async function create(){ const r=await fetch(`${API}/admin/discounts`,{ method:'POST', credentials:'include', headers:{'Content-Type':'application/json'}, body: JSON.stringify(form) }); if(r.ok){ setForm({ code:'', type:'PERCENT', value:10, active:true }); load(); } else alert('Create failed'); }
  async function update(c:any){ const r=await fetch(`${API}/admin/discounts/${c.id}`,{ method:'PATCH', credentials:'include', headers:{'Content-Type':'application/json'}, body: JSON.stringify(c) }); if(r.ok) load(); }
  async function del(id:number){ if(!confirm('Delete?')) return; const r=await fetch(`${API}/admin/discounts/${id}`,{ method:'DELETE', credentials:'include' }); if(r.ok) load(); }
  return (

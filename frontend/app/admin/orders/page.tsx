...
export default function AdminOrders(){
  ...
  async function load(){ const r=await fetch(`${API}/admin/orders`,{ credentials:'include' }); const d=await r.json(); setList(d.orders||[]); }
  useEffect(()=>{ load(); },[]);
  async function setStatus(id:number, status:string){ const r=await fetch(`${API}/admin/orders/${id}/status`,{ method:'POST', credentials:'include', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ status })}); if(r.ok) load(); else alert('Failed'); }
  return (
